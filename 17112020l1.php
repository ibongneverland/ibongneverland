...
class Site extends SiteModule
{
    ...
    private function _resultAntrian()
    {
        ...
        if ($header[$this->options->get('jkn_mobile.header')] == $this->_getToken()) {
			...
			$data_nik = $this->db('pasien')->where('no_ktp', $decode['nik'])->oneArray();
			...
			if($data_pasien == 0) {
               $errors[] = 'Peserta belum terdaftar di rumah sakit, silahkan hubungi rumah sakit untuk informasi lebih lanjut';
            }
			...
			if($data_nik == 0) {
               $errors[] = 'Nomor KTP belum terdaftar di rumah sakit, silahkan hubungi rumah sakit untuk informasi lebih lanjut';
            }
            if(empty($decode['nik'])) {
               $errors[] = 'Nomor kartu tidak boleh kosong';
            }
            if(!empty($decode['nik']) && mb_strlen($decode['nik'], 'UTF-8') < 16){
               $errors[] = 'Nomor KTP harus 16 digit atau format tidak sesuai';
            }
            ...
            if(!empty($decode['tanggalperiksa']) && $decode['tanggalperiksa'] == $cek_referensi['tanggal_periksa']) {
               $errors[] = 'Anda sudah terdaftar dalam antrian ditanggal '.$cek_referensi['tanggal_periksa'];
            }
